...
- **Phase 4b — Shopping list (next):** aggregate ingredients across the planned week into a checklist. Decided approach: a client-side parser for the common ingredient shapes with a verbatim raw-line fallback, merging by item+unit. Worth knowing before starting — `extract.py` fills `item`/`qty`/`unit` as `null` for every JSON-LD import, so only AI-structured recipes have parsed fields today and the parser carries most of the load. An AI backfill is the follow-up if the parser proves too lossy.
